...
export default function MenuLateral({ children }) {
    ...
    const opçõesLocador = [
        { label: "Página Inicial", command: () => navegar("/pagina-inicial") },
        {
            label: "Menu", items: [
                ...
                { label: "Cadastrar Locador", command: () => navegar("/cadastrar-locador") },
                { label: "Administrar Residências", command: () => navegar("/administrar-residências") },
                { label: "Sair do Sistema", command: () => sairSistema() }
            ]
        },
    ];

    const opçõesLocatário = [
        { label: "Página Inicial", command: () => navegar("/pagina-inicial") },
        {
            label: "Menu", items: [
                {
                ...
                { label: "Cadastrar Locatário", command: () => navegar("/cadastrar-locatário") },
                { label: "Administrar Interesses", command: () => navegar("/administrar-interesses") },
                { label: "Sair do Sistema", command: () => sairSistema() },
            ]
        },
    ];
    ...
    function opçõesMenu() {
        switch (usuárioLogado.perfil) {
            case "locador": return opçõesLocador;
            case "locatário": return opçõesLocatário;
            default: return;
        }
    };
