...
export const TAMANHOS = { CPF: 13, SENHA: 15, TELEFONE: 12, MAX_LINHAS_TABELA: 10 };
...
export function estilizarBotão() {
    ...
    return `p-button-sm h-2rem text-base w-auto md:w-min mr-2 bg-${cor_botão}-600 border-${cor_botão}-800 shadow-6`;
};

export function estilizarBotãoRemover() {
    const cor_borda = "bluegray";
    return `p-button-sm h-2rem text-base w-auto md:w-min mr-2 p-button-danger border-${cor_borda}-800 shadow-6`;
};
...
export function estilizarMenuLateralDesktop(cor_tema) {
    return `w-15rem p-2 flex flex-column align-items-center h-screen fixed surface-50 bg-${cor_tema}-100 text-${cor_tema}-800`;
};
